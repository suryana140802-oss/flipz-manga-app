...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/suryana140802-oss/flipz-manga-app.git
   ```
2. **Open in Android Studio** (Recommended: Android Studio Ladybug or later).
3. **Environment Configuration**:
   Copy `.env.example` to `.env` if custom API keys are required:
   ```bash
   cp .env.example .env
   ```
4. **Build & Run**:
   Select your connected device or emulator (API 24+) and click **Run** (`Shift + F10`).
